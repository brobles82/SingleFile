...
class Processor {
  ...
  initialize() {
    ...
    if (!this.options.removeImports && this.options.imports) {
      this.options.imports.forEach(
        (importData) => (this.maxResources += importData.maxResources || 0)
      );
    }
  ...
  async getPageData() {
    ...
    if (this.options.insertSingleFileComment) {
      ...
      if (
        ...
        const info =
          this.doc.documentElement.firstChild.textContent.split("\n");
    ...
    if (
      ...
      const extension =
        extensionMatch && extensionMatch[0] && extensionMatch[0].length > 1
          ? extensionMatch[0]
          : "";
      filename = await util.truncateText(
        filename,
            ...
            if (imageData) {
              if (
                ...
                if (this.options.loadDeferredImages) {
                  if (
                    (!imgElement.getAttribute("src") ||
                      imgElement.getAttribute("src") == EMPTY_IMAGE) &&
      ...
      this.doc.querySelectorAll("style").forEach((styleElement, styleIndex) => {
        ...
        if (attributeValue) {
          const stylesheetContent =
            this.options.stylesheets[Number(styleIndex)];
  ...
  removeScripts() {
    ...
    this.doc.querySelectorAll("[href]").forEach((element) => {
      if (
        element.href &&
        element.href.match &&
        element.href.match(/^\s*javascript:/)
      ) {
  ...
  removeVideoSources() {
    const videoSourceElements = this.doc.querySelectorAll(
  ...
  removeAudioSources() {
    const audioSourceElements = this.doc.querySelectorAll(
  ...
  removeDiscardedResources() {
    ...
    this.doc
      .querySelectorAll(
        'meta[http-equiv=refresh], meta[disabled-http-equiv], meta[http-equiv="content-security-policy"]'
      )
      .forEach((element) => element.remove());
  ...
  saveFavicon() {
    ...
    if (!faviconElement) {
      faviconElement = this.doc.querySelector(
  ...
  resolveStyleAttributeURLs() {
    this.doc.querySelectorAll("[style]").forEach((element) => {
      ...
      styleContent = ProcessorHelper.resolveStylesheetURLs(
        styleContent,
        this.baseURI,
  ...
  async resolveStylesheetURLs() {
    ...
    async function processElement(
      ...
      let stylesheetContent = await getStylesheetContent(
        element,
        baseURI,
        options,
        workStyleElement
      ...
      if (!matchCharsetEquals(stylesheetContent, options.charset)) {
        options = Object.assign({}, options, {
          charset: getCharset(stylesheetContent),
        });
        stylesheetContent = await getStylesheetContent(
          element,
          baseURI,
          options,
          workStyleElement
    ...
    async function getStylesheetContent(
      ...
      if (element.tagName == "LINK") {
        content = await ProcessorHelper.resolveLinkStylesheetURLs(
          element.href,
          baseURI,
          options,
          workStyleElement
        ...
        content = await ProcessorHelper.resolveImportURLs(
          element.textContent,
          baseURI,
          options,
          workStyleElement
        ...
        if (attributeValue) {
          ...
          if (shadowRootData) {
            ...
            templateElement.setAttribute(
              SHADOW_MODE_ATTRIBUTE_NAME,
              shadowRootData.mode
            );
            if (shadowRootData.delegatesFocus) {
              templateElement.setAttribute(
                SHADOW_DELEGATE_FOCUS_ATTRIBUTE_NAME,
                ""
              );
            }
  ...
  async resolveHtmlImportURLs() {
    const linkElements = Array.from(
      this.doc.querySelectorAll("link[rel=import][href]")
    );
    await Promise.all(
      linkElements.map(async (linkElement) => {
        ...
        const attributeValue = linkElement.getAttribute(
          util.HTML_IMPORT_ATTRIBUTE_NAME
        );
        if (attributeValue) {
          ...
          if (importData) {
            ...
            if (!options.removeImports) {
              importData.maxResources =
                importData.runner.batchRequest.getMaxResources();
            }
  ...
  async processStyleAttributes() {
    return Promise.all(
      [...this.styles].map(([, declarationList]) =>
        ProcessorHelper.processStyle(
          declarationList.children.toArray(),
          this.baseURI,
          this.options,
          this.cssVariables,
          this.batchRequest
  ...
  async processHtmlImports() {
    const linkElements = Array.from(
      this.doc.querySelectorAll("link[rel=import]")
    );
    await Promise.all(
      linkElements.map(async (linkElement) => {
        const attributeValue = linkElement.getAttribute(
        ...
        if (stylesheetInfo) {
          ...
          if (this.options.saveOriginalURLs) {
            ...
            styleElement.setAttribute(
              ...
              linkElement.getAttribute("data-sf-original-href")
            );
          }
          styleElement.textContent = stylesheetContent;
  ...
  resetZoomLevel() {
    ...
    const transformOriginPriority =
      this.doc.documentElement.style.getPropertyPriority(
        "-sf-transform-origin"
      );
    const minHeight =
...
class ProcessorHelper {
  ...
  static async resolveImportURLs(
    ...
    stylesheetContent = ProcessorHelper.resolveStylesheetURLs(
      stylesheetContent,
      baseURI,
      workStylesheet,
      options.saveOriginalURLs
    );
        ...
        if (match) {
          ...
          if (!testIgnoredPath(resourceURL) && testValidPath(resourceURL)) {
            ...
            if (
              ...
              if (options.compressCSS) {
                importedStylesheetContent = util.compressCSS(
                  importedStylesheetContent
                );
              }
              importedStylesheetContent = wrapMediaQuery(
                importedStylesheetContent,
                match.media
              );
              if (stylesheetContent.includes(cssImport)) {
                ...
                if (
                  ...
                  (!workStylesheet.sheet && importedStylesheetContent)
                ) {
    ...
    async function getStylesheetContent(resourceURL) {
      ...
      if (
        !matchCharsetEquals(content.data, content.charset || options.charset)
      ) {
        options = Object.assign({}, options, {
    ...
    urlFunctions.map((urlFunction) => {
      ...
      if (!testIgnoredPath(resourceURL)) {
        if (!resourceURL || testValidPath(resourceURL)) {
          ...
          if (
            testValidURL(resolvedURL) &&
            originalResourceURL != resolvedURL &&
            stylesheetContent.includes(urlFunction)
          ) {
            try {
              stylesheetContent = stylesheetContent.replace(
                getRegExp(urlFunction),
                originalResourceURL
                  ? urlFunction.replace(originalResourceURL, resolvedURL)
                  : "url(" + resolvedURL + ")"
              );
            } catch (error) {
              // ignored
          ...
          if (originalResourceURL) {
            newUrlFunction = urlFunction.replace(
              originalResourceURL,
              EMPTY_DATA_URI
            );
          } else {
  ...
  static async resolveLinkStylesheetURLs(
    ...
    if (
      ...
      stylesheetContent = await ProcessorHelper.resolveImportURLs(
        stylesheetContent,
        resourceURL,
        options,
        workStylesheet
    ...
    for (let cssRule = cssRules.head; cssRule; cssRule = cssRule.next) {
      ...
      if (ruleData.type == "Atrule" && ruleData.name == "charset") {
        ...
        if (ruleData.type == "Rule") {
          promises.push(
            this.processStyle(
              ruleData.block.children.toArray(),
              baseURI,
              options,
              cssVariables,
              batchRequest
    ...
    async function processFontFaceRule(ruleData, fontURLs) {
      await Promise.all(
        ruleData.block.children.toArray().map(async (declaration) => {
          if (declaration.type == "Declaration" && declaration.value.children) {
            const urlFunctions = getUrlFunctions(
              getCSSValue(declaration.value),
              true
            );
            await Promise.all(
      ...
      function replaceURLs(declaration, oldURL, newURL) {
        declaration.value.children.forEach((token) => {
          if (
            token.type == "Url" &&
            util.removeQuotes(getCSSValue(token.value)) == oldURL
          ) {
  ...
  static async processStyle(
    ...
    await Promise.all(
      declarations.map(async (declaration) => {
        if (
          ...
          try {
            declaration.value = cssTree.parse(declaration.value.value, {
              context: "value",
            });
          } catch (error) {
            // ignored
              ...
              if (!testIgnoredPath(resourceURL)) {
                if (testValidURL(resourceURL)) {
                  ...
                  findURLToken(
                    originalResourceURL,
                    declaration.value.children,
                    (token, parent, rootFunction) => {
                      if (!originalResourceURL.startsWith("#")) {
                        if (
                          duplicate &&
                          options.groupDuplicateImages &&
                          rootFunction &&
                          util.getContentSize(content) <
                            SINGLE_FILE_VARIABLE_MAX_SIZE
                        ) {
                          const value = cssTree.parse(
                            "var(" +
                  ...
                  if (variableDefined) {
                    ...
                    tokens.forEach(({ parent, token, value }) =>
                      parent.replace(token, value)
                    );
                  }
                }
              }
    ...
    function findURLToken(url, children, callback, depth = 0) {
      for (let token = children.head; token; token = token.next) {
        ...
        if (
          token.data.type == "Url" &&
          util.removeQuotes(getCSSValue(token.data.value)) == url
        ) {
...
const REGEXP_URL_FN =
  /(url\s*\(\s*'(.*?)'\s*\))|(url\s*\(\s*"(.*?)"\s*\))|(url\s*\(\s*(.*?)\s*\))/gi;
const REGEXP_URL_SIMPLE_QUOTES_FN = /^url\s*\(\s*'(.*?)'\s*\)$/i;
const REGEXP_URL_DOUBLE_QUOTES_FN = /^url\s*\(\s*"(.*?)"\s*\)$/i;
const REGEXP_URL_NO_QUOTES_FN = /^url\s*\(\s*(.*?)\s*\)$/i;
const REGEXP_IMPORT_FN =
  /(@import\s*url\s*\(\s*'(.*?)'\s*\)\s*(.*?)(;|$|}))|(@import\s*url\s*\(\s*"(.*?)"\s*\)\s*(.*?)(;|$|}))|(@import\s*url\s*\(\s*(.*?)\s*\)\s*(.*?)(;|$|}))|(@import\s*'(.*?)'\s*(.*?)(;|$|}))|(@import\s*"(.*?)"\s*(.*?)(;|$|}))|(@import\s*(.*?)\s*(.*?)(;|$|}))/gi;
const REGEXP_IMPORT_URL_SIMPLE_QUOTES_FN =
  /@import\s*url\s*\(\s*'(.*?)'\s*\)\s*(.*?)(;|$|})/i;
const REGEXP_IMPORT_URL_DOUBLE_QUOTES_FN =
  /@import\s*url\s*\(\s*"(.*?)"\s*\)\s*(.*?)(;|$|})/i;
const REGEXP_IMPORT_URL_NO_QUOTES_FN =
  /@import\s*url\s*\(\s*(.*?)\s*\)\s*(.*?)(;|$|})/i;
...
function findShortcutIcon(shortcutIcons) {
  shortcutIcons = shortcutIcons.filter(
    (linkElement) => linkElement.href != EMPTY_IMAGE
  );
  shortcutIcons.sort(
...
function matchURL(stylesheetContent) {
  const match =
    stylesheetContent.match(REGEXP_URL_SIMPLE_QUOTES_FN) ||
...
function addOriginalURLs(stylesheetContent) {
  return stylesheetContent.replace(
    REGEXP_URL_FN,
    function (match, _0, url, _1, url2, _2, url3) {
      url = url || url2 || url3;
      if (isDataURL(url)) {
        return match;
      } else {
...
function removeCssComments(stylesheetContent) {
  try {
    ...
    do {
      ...
      if (start != -1 && end != -1) {
        stylesheetContent =
          stylesheetContent.substring(0, start) +
          stylesheetContent.substr(end + 2);
      }
    } while (start != -1 && end != -1);
    return stylesheetContent;
  }
