...
async function initialize(options) {
  ...
  if (options.crawlSyncSession || options.crawlLoadSession) {
    try {
      tasks = JSON.parse(
        fs
          .readFileSync(options.crawlSyncSession || options.crawlLoadSession)
          .toString()
      );
    } catch (error) {
...
async function runTasks() {
  ...
  const processingTasks = tasks.filter(
    (task) => task.status == STATE_PROCESSING
  ).length;
  const promisesTasks = [];
  for (
    let workerIndex = 0;
    workerIndex <
    Math.min(availableTasks, maxParallelWorkers - processingTasks);
    workerIndex++
  ) {
...
function createTask(url, options, parentTask, rootTask) {
  ...
  if (VALID_URL_TEST.test(url)) {
    ...
    const isChild =
      ...
      rootBaseURIMatch &&
      rootBaseURIMatch[1] &&
      url.startsWith(rootBaseURIMatch[1]);
    return {
      url,
      isInnerLink,
      isChild,
      ...
      externalLinkDepth: isInnerLink
        ? -1
        : parentTask
        ? parentTask.externalLinkDepth + 1
        : -1,
      options,
    };
  }
      ...
      JSON.stringify(
        tasks.map((task) =>
          Object.assign({}, task, {
            ...
            promise: undefined,
            options:
              task.status && task.status == STATE_PROCESSED
                ? undefined
                : task.options,
          })
        )
